...
class RenaissanceResult:
    FILENAME_REGEX = re.compile(r"(?P<duet_name>[a-zA-Z0-9_-]+)\.(?P<runid>\d+)\.(?P<duet_type>[AB])\.results.json")
    ...
    def __init__(self, result_path: os.path, config: str):
        ...
        if not match:
            logging.warning(f"Failed to match `{self.result_path}` with `{self.FILENAME_REGEX}`")
    ...
    def convert_results_json(self) -> pd.DataFrame:
        ...
        if len(results_json["data"]) != 1:
            logging.warning(f"Duet run expects only single running benchmark, results contain these: {results_json['data'].keys()}")
        ...
        logging.info(f"Processed file: {self.result_path}, runid: {self.runid}, pair: {self.pair}, total_iterations: {total_iterations}")
...
def process_renaissance(results_dir: os.path, config: DuetBenchConfig) -> pd.DataFrame:
    renaissance_results = [RenaissanceResult(os.path.join(results_dir, file), config) for file in os.listdir(results_dir)]
...
def main():
    ...
    parser.add_argument("results", type=str, help="Results directory of Renaissance Duet")
    parser.add_argument("-c", "--config", type=str, required=True, help="Duet config file")
    parser.add_argument("-o", "--output", type=str, help="Output results.csv file, default <results_dir_name>.csv")
